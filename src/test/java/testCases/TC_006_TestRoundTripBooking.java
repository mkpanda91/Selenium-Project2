...
public class TC_006_TestRoundTripBooking extends BaseDriverClass {
	...
	@Feature("Booking")
	@Description("This test attempts to test the functionality of booking round trip flight tickets")
	public void testRoundWayBooking() {
		try {
			// Home Page Interactions
			home_pgObj = new HomePage(driver);
			home_pgObj.setRoundTrip();
			...
			//onward journey date setup
			home_pgObj.setDeptDate(p.getProperty("OnwardTravelDay"), p.getProperty("OnwardTravelMonth"),
					p.getProperty("OnwardTravelYear"));
			
			//return journey date setup
			home_pgObj.setReturnDate(p.getProperty("ReturnTravelDay"), p.getProperty("ReturnTravelMonth"),
					p.getProperty("ReturnTravelYear"));
			...
			sr_pgObj.selectReturnFlight(p.getProperty("ReturnFaretype"));
			...
			addon_pgObj.chooseTwowayTripAddOn(p.getProperty("OnwardFaretype"), p.getProperty("ReturnFaretype"), p.getProperty("AdultPaxCount"), p.getProperty("ChildPaxCount"));
